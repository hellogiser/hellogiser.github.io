...
- [空间插值](#空间插值)
  - [空间插值的确定性方法](#空间插值的确定性方法)
    ...
    - [多项式趋势面](#多项式趋势面)
      ...
      - [二次拟合](#二次拟合)
      - [绘图](#绘图-1)
    - [邻近度分析-泰森多边形](#邻近度分析-泰森多边形)
      - [绘图](#绘图-2)
    - [最近邻插值](#最近邻插值)
      - [转换为栅格](#转换为栅格)
      - [绘制最近邻预测 SOC](#绘制最近邻预测-soc)
    - [反距离加权插值](#反距离加权插值)
      - [转换为栅格](#转换为栅格-1)
      - [绘制 IDW 预测的 SOC](#绘制-idw-预测的-soc)
    - [薄板样条插值](#薄板样条插值)
  - [地统计学插值](#地统计学插值)
    - [半变异函数建模](#半变异函数建模)
        - [加载包和数据](#加载包和数据)
      ...
      - [拟合变异函数模型](#拟合变异函数模型)
        ...
        - [球面(Sph)模型](#球面sph模型)
      - [拟合优度](#拟合优度)
      - [用变换后的数据进行变异函数建模](#用变换后的数据进行变异函数建模)
        - [Log10转换](#log10转换)
        - [幂变换 (Box-Cox)](#幂变换-box-cox)
      - [嵌套模型拟合](#嵌套模型拟合)
        - [短程结构](#短程结构)
        - [远程结构](#远程结构)
    - [克里金插值](#克里金插值)
      - [普通克里金](#普通克里金)
        ...
        - [数据变换](#数据变换)
        - [变异函数建模](#变异函数建模)
          - [绘制变量图和拟合模型](#绘制变量图和拟合模型)
        - [点或准时克里金法](#点或准时克里金法)
          - [逆变换](#逆变换)
          - [转换为栅格](#转换为栅格-2)
          - [绘制预测 SOC 和 OK 误差](#绘制预测-soc-和-ok-误差)
        - [块克里金法](#块克里金法)
          - [逆变换](#逆变换-1)
          - [转换为栅格](#转换为栅格-3)
          - [绘图](#绘图-3)
      - [泛克里金](#泛克里金)
        - [数据变换](#数据变换-1)
        - [变异函数建模](#变异函数建模-1)
          - [绘制残差方差图和拟合模型](#绘制残差方差图和拟合模型)
        - [克里金预测](#克里金预测)
          - [逆变换](#逆变换-2)
          - [转换为栅格](#转换为栅格-4)
          - [绘制预测 SOC 和 UK 误差](#绘制预测-soc-和-uk-误差)
      - [协同克里金](#协同克里金)
          - [幂变换](#幂变换)
          - [相关矩阵](#相关矩阵)
        - [协区域化或 Corss-Varoigram 的变异函数模型](#协区域化或-corss-varoigram-的变异函数模型)
          - [目标变量直接方差图（SOC）](#目标变量直接方差图soc)
          - [协变量变异函数建模的直接变异函数(NDVI)](#协变量变异函数建模的直接变异函数ndvi)
          - [交叉变异函数](#交叉变异函数)
          - [网格位置的协同克里金法预测](#网格位置的协同克里金法预测)
          - [逆变换](#逆变换-3)
          - [转换为栅格](#转换为栅格-5)
          - [绘制预测 SOC 和 CK 误差](#绘制预测-soc-和-ck-误差)
